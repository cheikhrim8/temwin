            ...
            <fieldset @if(!(Auth::user()->hasAccess(2,2)) or $etat !=0)  disabled="true" @endif>
                <div class="row">
                    ...
                    <div class="col-md-12 form-group">
                        <label for="content_value">{{ trans('text_me.content_value_fr') }}</label>
                        <textarea id="content_value" name="content_value"
                                  class="form-control">{{$element->content_value}}</textarea>
                    </div>
                    <div class="col-md-12 form-group">
                        <label for="content_value_ar">{{ trans('text_me.content_value_ar') }}</label>
                        <textarea id="content_value_ar" name="content_value_ar"
                                  class="form-control">{{$element->content_value_ar}}</textarea>
                    </div>


                    <div class="col-md-4 form-group">
                        <label for="position">{{ trans('text_me.type_insertion') }}<span class="required_field"
                                                                                         data-toggle="tooltip"
                                                                                         data-placement="right"
                                                                                         title="{{ trans('text.champ_obligatoire') }}">*</span></label>
                        <select id="positionedit" name="position" class="form-control"
                                onchange="positionElementedit({{$element->modeles_acte_id}} , '{{ trans('text_me.premier') }}' , '{{ trans('text_me.dernier') }}' , '{{ trans('text_me.apres') }}')">
                            @if($ligne !='')
                    ...
                    <div class="col-md-4 form-group">
                        <label for="ligneedit">{{ trans('text_me.position') }}<span class="required_field"
                                                                                    data-toggle="tooltip"
                                                                                    data-placement="right"
                                                                                    title="{{ trans('text.champ_obligatoire') }}">*</span></label>
                        <select id="ligneedit" name="ligne" class="form-control"
                                onchange="parentLigneedit({{$element->modeles_acte_id}}, '{{ trans('text_me.premier') }}' , '{{ trans('text_me.dernier') }}' , '{{ trans('text_me.apres') }}','{{ trans('text_me.suite_elemet') }}','{{ trans('text_me.pas_de_parent') }}')">
                            @if($ligne !='')
                                ...
                                @foreach($positions as $pos)
                                    ...
                                    @else
                                        <option value="{{$pos->ligne}}">{{ trans('text_me.apres') }}
                                            "{{$pos->content_value}}"
                                        </option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-md-4 form-group" id="divparentedit" @if($ligne !='') style="display: none;" @endif>
                        ...
                        <select id="parentedit" name="parent" class="form-control"
                                onchange="parentElementsedit({{$element->modeles_acte_id}}, '{{ trans('text_me.premier') }}' , '{{ trans('text_me.dernier') }}' , '{{ trans('text_me.apres') }}')">
                            @if($parent!='')
                                @foreach($parents as $p)
                                    @if($parent->id==$p->id)
                                        <option value="{{$p->id}}" selected>{{$p->content_value}}</option>
                                    @else
                                        <option value="{{$p->id}}">{{ trans('text_me.suite_elemet') }}
                                            "{{$p->content_value}}"
                                        </option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-md-4 form-group" id="divordresedit">
                        <label for="ordresedit">{{ trans('text_me.ordre_affichage') }}</label><span
                        ...
                        <select id="ordresedit" name="ordres" class="form-control">
                            @if($ligne!='<br>')
                                ...
                                @foreach($ordres as $ord)
                                    ...
                                    @else
                                        <option value="{{$ord->id}}">{{ trans('text_me.apres') }}
                                            "{{$ord->content_value}}"
                                        </option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-md-4 form-group"  id="type_ligne" @if($ligne =='') style="display: none;" @endif>
                        <label for="type_ligne">{{ trans('text_me.type') }}</label>
                        <select id="type_ligne" readonly="" name="type" class="form-control" >
                            <option value="0" @if($element->type==0) selected="selected" @endif>Normal</option>
                            <option value="1" @if($element->type==1) selected="selected" @endif>Tableau</option>
                    ...
                    <div class="col-md-4 form-group" id="varriable"
                         @if($element->nature_content==1)  @else style="display:none;" @endif>
                    ...
                    <div class="col-md-8 form-group" id="divchoix"
                         @if($element->type_content==3)  @else style="display:none;" @endif>
                    ...
                    <input id="id_element" name="id_element" type="hidden" class="form-control"
                           value="{{ $element->id }}">
                    <input id="type" name="type" type="hidden" class="form-control"
                           value="{{ $element->type_content }}">
                    <input id="nature" name="nature" type="hidden" class="form-control"
                           value="{{ $element->nature_content }}">
                    ...
                    <div class="col-md-12 form-row">
                        ...
                        <div class="col-md-4 form-group text-right">
                            <button class="btn btn-success btn-icon-split" onclick="editElementActe(this)"
